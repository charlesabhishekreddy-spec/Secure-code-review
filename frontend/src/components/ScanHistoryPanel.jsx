...
export function ScanHistoryPanel({
  ...
  const canCompare = Boolean(currentResults && selectedEntry);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-2xl font-semibold text-white">
            {currentResults ? "Compare the current scan with earlier runs" : "Review saved scans and reopen earlier runs"}
          </h2>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <article className="rounded-3xl border border-white/10 bg-slate-950/30 p-5">
            ...
            <p className="mt-3 text-3xl font-black text-white">
              {canCompare ? compareDelta(currentResults?.security_score, selectedEntry.results?.security_score) : "n/a"}
            </p>
